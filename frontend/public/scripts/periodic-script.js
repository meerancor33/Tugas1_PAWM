...
function createPeriodicTable() {
  ...
  const mainElements = elements.filter((el) => el.row <= 7 && el.row >= 1)

  // Sort elements by row and column for proper positioning
  ...
  for (let row = 0; row < 7; row++) {
    for (let col = 0; col < 18; col++) {
      ...
      if (element) {
        ...
        elementDiv.style.gridRow = row + 1
        elementDiv.style.gridColumn = col + 1
        table.appendChild(elementDiv)
      }
    }
  }

  // Add lanthanides series
  const lanthanideContainer = document.createElement("div")
  lanthanideContainer.className = "lanthanide-series"
  lanthanideContainer.style.gridRow = "9"
  lanthanideContainer.style.gridColumn = "1 / -1"

  const lanthanideLabel = document.createElement("div")
  lanthanideLabel.className = "series-label"
  lanthanideLabel.textContent = "Lantanida"
  lanthanideContainer.appendChild(lanthanideLabel)

  // Add La first
  const laElement = elements.find((el) => el.number === 57)
  lanthanideContainer.appendChild(createElementDiv(laElement))

  // Add lanthanides (Ce to Lu)
  const lanthanides = elements.filter((el) => el.number >= 58 && el.number <= 71)
  lanthanides.sort((a, b) => a.number - b.number)
  lanthanides.forEach((element) => {
    lanthanideContainer.appendChild(createElementDiv(element))
  })

  table.appendChild(lanthanideContainer)

  // Add actinides series
  const actinideContainer = document.createElement("div")
  actinideContainer.className = "actinide-series"
  actinideContainer.style.gridRow = "10"
  actinideContainer.style.gridColumn = "1 / -1"

  const actinideLabel = document.createElement("div")
  actinideLabel.className = "series-label"
  actinideLabel.textContent = "Aktinida"
  actinideContainer.appendChild(actinideLabel)

  // Add Ac first
  const acElement = elements.find((el) => el.number === 89)
  actinideContainer.appendChild(createElementDiv(acElement))

  // Add actinides (Th to Lr)
  const actinides = elements.filter((el) => el.number >= 90 && el.number <= 103)
  actinides.sort((a, b) => a.number - b.number)
  actinides.forEach((element) => {
    actinideContainer.appendChild(createElementDiv(element))
  })

  table.appendChild(actinideContainer)
}
...
function showElementInfo(element) {
  ...
  document.getElementById("infoNumber").textContent = element.number
  document.getElementById("infoMass").textContent = element.mass + " u"
  document.getElementById("infoCategory").textContent = categoryNames[element.category]
...
// Close element info when clicking outside
document.addEventListener("click", (event) => {
  const infoPanel = document.getElementById("elementInfo")
  const isClickInsideInfo = infoPanel.contains(event.target)
  const isClickOnElement = event.target.closest(".element")

  if (!isClickInsideInfo && !isClickOnElement && infoPanel.classList.contains("show")) {
    closeElementInfo()
  }
})

// Export data and functions for external use
console.log("Periodic Table Script Loaded!")
console.log(
  "Available functions: getElementById(), getElementBySymbol(), getElementsByCategory(), getElementsByPeriod(), getElementsByGroup()",
)
